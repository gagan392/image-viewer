...
class Profile extends Component {
	...
	async componentWillMount() {
		...
		this.setState({ profile_picture: profileData.data.profile_picture });
		this.setState({ username: profileData.data.username });
		this.setState({ full_name: profileData.data.full_name });
		this.setState({ media: profileData.data.counts.media });
		this.setState({ follows: profileData.data.counts.follows });
		this.setState({ followed_by: profileData.data.counts.followed_by });

		const postsData = await apiClient.getMdeia();
		this.setState({ uploaded_pics: postsData.data });
		this.setState({ likes: postsData.data[0].likes });
		this.setState({ url: postsData.data[0].images.standard_resolution.url });
		this.setState({ id: postsData.data[0].id });
